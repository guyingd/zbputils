...
import (
	...
	"github.com/guyingd/zbputils/ctxext"
	"github.com/guyingd/zbputils/img/text"
)
...
func init() {
	process.NewCustomOnce(&managers).Do(func() {
		...
		zero.OnCommandGroup([]string{"菜单", "service_list"}, zero.UserOrGrpAdmin).SetBlock(true).SecondPriority().
			Handle(func(ctx *zero.Ctx) {
				err := geticonandfont()
				if err != nil {
					ctx.SendChain(message.Text("ERROR: ", err))
		...
		zero.OnCommand("设置菜单显示行数", zero.SuperUserPermission).SetBlock(true).SecondPriority().Handle(func(ctx *zero.Ctx) {
			model := extension.CommandModel{}
			_ = ctx.Parse(&model)
